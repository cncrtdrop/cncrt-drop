        ...
        <div className="grid sm:grid-cols-2 gap-8 sm:gap-12">
          <div>
            {(() => {
              ...
              return (
                <>
                  <div className="relative aspect-[4/5] border border-neutral-200 bg-stone-50 overflow-hidden">
                    {images[galleryIndex] && (
                      <Image src={images[galleryIndex]} alt={product.name} fill className="object-cover" />
                    )}
                    {product.tag && (
                      <span className="absolute top-2 left-2 bg-white/90 text-neutral-900 text-[10px] font-bold uppercase tracking-widest px-2 py-1">
                        {product.tag}
                      </span>
                    )}
                    {hasMultiple && (
                      <>
                        <button
                          onClick={prev}
                          className="absolute left-2 top-1/2 -translate-y-1/2 bg-white/90 text-neutral-900 p-1.5"
                        >
                          <ChevronLeft size={16} />
                        </button>
                        <button
                          onClick={next}
                          className="absolute right-2 top-1/2 -translate-y-1/2 bg-white/90 text-neutral-900 p-1.5"
                        >
                    ...
                    <div className="flex gap-2 mt-3">
                      {images.map((img, i) => (
                        <button
                          ...
                          className={
                            "w-14 h-16 border overflow-hidden relative " +
          ...
          <div className="flex flex-col">
            <span className="w-fit text-[9px] font-bold uppercase tracking-widest text-pink-700 border border-pink-600/40 px-2 py-0.5 mb-2">
            ...
            >
              {added ? "Ajouté ✓ — voir le panier sur le catalogue" : "Ajouter au panier"}
            </button>
            {added && (
              <Link href="/" className="mt-2 text-xs text-center underline text-neutral-500">
                Retour au catalogue pour payer
              </Link>
            )}
            ...
            <div className="mt-8 border-t border-dashed border-neutral-300 pt-4 text-xs text-neutral-500 leading-relaxed uppercase tracking-widest">
              ...
              <div className="flex items-center gap-1"><Check size={12} /> CB / Apple Pay — zéro frais acheteur</div>
            </div>
          </div>
        </div>
      </div>
