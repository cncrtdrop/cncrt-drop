...
export default function Home() {
  ...
  const addToCart = (product) => {
    setCart(addToCartStorage(product));
    setCartOpen(true);
  };
  ...
  const searchResults = searchQuery.trim()
    ? products.filter((p) =>
        (p.brand + " " + p.name).toLowerCase().includes(searchQuery.trim().toLowerCase())
      )
    : [];
        ...
        <section className="max-w-5xl mx-auto px-4 sm:px-6 py-12">
          <div className="flex items-baseline justify-between mb-6">
            ...
            <span className="text-[10px] sm:text-xs text-neutral-500 uppercase tracking-widest">
              {(isSearching ? searchResults : filteredProducts).length} pièces
            </span>
          </div>
                ...
                <Link key={p.id} href={`/product/${p.id}`}>
                  <div className="bg-white border border-neutral-200 flex flex-col cursor-pointer hover:border-neutral-900 transition-colors">
                    <div className="relative aspect-[4/5] overflow-hidden bg-stone-100">
                      ...
                      {p.tag && (
                        <span className="absolute top-2 left-2 bg-white/90 text-neutral-900 text-[9px] sm:text-[10px] font-bold uppercase tracking-widest px-2 py-1">
                          {p.tag}
                        </span>
                      )}
                    </div>
            ...
            <div className="relative w-full max-w-sm bg-white h-full border-l border-neutral-300 flex flex-col">
              ...
              <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-3">
                {cart.length === 0 ? (
                  ...
                  cart.map((item, i) => (
                    <div key={i} className="flex items-center gap-3 bg-stone-50 border border-neutral-200 p-2">
                      {item.image_url && (
                      ...
                      <button onClick={() => removeFromCart(i)} className="text-neutral-400 hover:text-pink-600">
                        <X size={16} />
                      </button>
                    </div>
                  ))
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 sm:px-6 py-10">
      ...
      <div className="flex flex-col gap-6 text-sm sm:text-base text-neutral-700 leading-relaxed max-w-xl font-sans-display">
        <p>
          CNCRT DROP est né d'une conviction simple : le vêtement le plus responsable
          est celui qu'on ne fabrique pas. Pas de nouvelle production, pas de nouveau
          stock jeté dans le monde — juste des pièces qui existent déjà, choisies avec
          exigence, et qu'on remet en circulation.
        </p>

        <div>
          <h2 className="font-bold uppercase tracking-widest text-xs text-neutral-900 mb-2">Comment on sélectionne</h2>
          <p>
            Chaque pièce est cueillie à la main pour deux raisons, jamais une seule :
            le style, et la qualité réelle du vêtement. On vérifie l'état, la matière,
            la coupe, avant de la proposer.
          </p>
        </div>

        <div>
          <h2 className="font-bold uppercase tracking-widest text-xs text-neutral-900 mb-2">Pourquoi la seconde main</h2>
          <p>
            On n'est pas ici pour faire la morale. Mais chaque pièce achetée d'occasion,
            c'est une pièce de moins produite quelque part. C'est un geste simple, pas un dogme.
          </p>
        </div>

        <div>
          <h2 className="font-bold uppercase tracking-widest text-xs text-neutral-900 mb-2">L'esprit du catalogue</h2>
          <p>
            Trois familles : les <strong>Bangers</strong> pour les pièces rares, les{" "}
            <strong>Classiques</strong> pour les indispensables increvables, et les{" "}
            <strong>Seconds</strong> pour les pièces avec un petit défaut et un prix qui va avec.
          </p>
        </div>

        <p className="text-neutral-500 italic">Des roses qui poussent là où on ne les attend pas.</p>
      </div>
    </div>
  );
}
