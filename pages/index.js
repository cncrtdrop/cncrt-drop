...
import { normalizeText, isEuropeCountry, extractType, extractSize, compareSizes } from "../lib/productFacets";

const CATEGORIES = [
  { id: "all", label: "Tous", description: "Toutes les pièces du catalogue, toutes catégories confondues." },
...
export default function Home() {
  ...
  const [activeCategory, setActiveCategory] = useState(() => {
    ...
    return sessionStorage.getItem("cncrt_activeCategory") || "all";
  });
  ...
  useEffect(() => {
    if (router.isReady && router.query.cart === "open") {
      setCartOpen(true);
  ...
  useEffect(() => {
    if (!justAddedToast) return;
    const t = setTimeout(() => setJustAddedToast(false), 4000);
    return () => clearTimeout(t);
  }, [justAddedToast]);
  ...
  const [sortOption, setSortOption] = useState(() => {
    ...
    return sessionStorage.getItem("cncrt_sortOption") || "default";
  });
  const [filterBrand, setFilterBrand] = useState(() => {
    if (typeof window === "undefined") return "all";
    return sessionStorage.getItem("cncrt_filterBrand") || "all";
  });
  const [filterType, setFilterType] = useState(() => {
    if (typeof window === "undefined") return "all";
    return sessionStorage.getItem("cncrt_filterType") || "all";
  });
  const [filterOrigin, setFilterOrigin] = useState(() => {
    if (typeof window === "undefined") return "all";
    return sessionStorage.getItem("cncrt_filterOrigin") || "all";
  });
  const [filterSize, setFilterSize] = useState(() => {
    if (typeof window === "undefined") return "all";
    return sessionStorage.getItem("cncrt_filterSize") || "all";
  });

  // Retient l'onglet / les filtres / le tri pendant la session, pour que
  // "retour" depuis une fiche produit retombe exactement là où on était.
  useEffect(() => {
    sessionStorage.setItem("cncrt_activeCategory", activeCategory);
  }, [activeCategory]);
  useEffect(() => {
    sessionStorage.setItem("cncrt_sortOption", sortOption);
  }, [sortOption]);
  useEffect(() => {
    sessionStorage.setItem("cncrt_filterBrand", filterBrand);
  }, [filterBrand]);
  useEffect(() => {
    sessionStorage.setItem("cncrt_filterType", filterType);
  }, [filterType]);
  useEffect(() => {
    sessionStorage.setItem("cncrt_filterOrigin", filterOrigin);
  }, [filterOrigin]);
  useEffect(() => {
    sessionStorage.setItem("cncrt_filterSize", filterSize);
  }, [filterSize]);
  ...
  const sizeLabels = new Map(); // clé normalisée -> libellé affiché (le premier rencontré)
  products.forEach((p) => {
    const s = extractSize(p.size);
    const key = normalizeText(s);
    if (key && !sizeLabels.has(key)) sizeLabels.set(key, s);
  });
  const allSizes = [...sizeLabels.values()].sort(compareSizes);

  const typeLabels = new Map(); // clé normalisée -> libellé affiché (le premier rencontré)
  products.forEach((p) => {
    const t = extractType(p.name);
    const key = normalizeText(t);
    if (key && !typeLabels.has(key)) typeLabels.set(key, t);
  });
  const allTypes = [...typeLabels.values()].sort((a, b) => a.localeCompare(b));

  const activeFacetCount = [filterBrand, filterType, filterOrigin, filterSize].filter((f) => f !== "all").length;
  ...
  const applyFacets = (list) =>
    list.filter(
      (p) =>
        (filterBrand === "all" || p.brand === filterBrand) &&
        (filterType === "all" || normalizeText(extractType(p.name)) === normalizeText(filterType)) &&
        (filterOrigin === "all" ||
          (filterOrigin === "Europe" ? isEuropeCountry(p.origin_country) : !isEuropeCountry(p.origin_country))) &&
        (filterSize === "all" || normalizeText(extractSize(p.size)) === normalizeText(filterSize))
    );

  const filteredProducts = applySort(
    applyFacets(activeCategory === "all" ? products : products.filter((p) => p.category === activeCategory))
  );

  const searchResults = searchQuery.trim()
    ? applySort(applyFacets(products.filter((p) => normalizeText(p.brand + " " + p.name).includes(normalizeText(searchQuery)))))
    : [];
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      alert("Erreur au paiement : " + err.message);
    }
    ...
    <div className={"transition-opacity duration-700 " + (siteVisible ? "opacity-100" : "opacity-0")}>
      ...
      <div className="min-h-screen bg-stone-100 text-neutral-900 font-mono">
        ...
        <section className="relative bg-neutral-950 text-stone-100 border-b border-neutral-800">
          <div className="relative w-full aspect-[4/5] max-w-md mx-auto sm:max-w-none">
            ...
            <div className="absolute inset-x-0 bottom-0 px-5 sm:px-8 pb-10 sm:pb-10 max-w-2xl">
              ...
              <p className="mt-2 text-pink-400 text-[9px] sm:text-[10px] uppercase tracking-widest font-bold">
                Livraison offerte en France métropolitaine dès 30€ d'achat
              </p>
            </div>
          </div>
        </section>
          ...
          <div className="max-w-5xl mx-auto pt-6 pb-4">
            <div className="flex items-baseline justify-between px-4 sm:px-6 mb-3">
              <h2 className="text-sm sm:text-base font-sans-display font-bold uppercase tracking-widest">
                Nouveaux <span className="text-pink-600">Drops</span>
              </h2>
                ...
                <Link key={p.id} href={`/product/${p.id}`} className="shrink-0 w-28 sm:w-32 text-left">
                  <div className="relative aspect-[4/5] bg-stone-100 border border-neutral-200 overflow-hidden">
                    {p.image_url ? (
                      ...
                      <div className="w-full h-full flex items-center justify-center text-[9px] text-neutral-400 uppercase tracking-widest text-center px-2">
                        Pas encore de photo
                      </div>
                    )}
                    {p.tag && (
        ...
        <section className="max-w-5xl mx-auto px-4 sm:px-6 py-12">
              ...
              <div className="flex gap-6 border-b border-dashed border-neutral-300 mb-2">
                {CATEGORIES.map((cat) => {
                  ...
                  return (
                    ...
                    >
                      {cat.label} <span className="font-normal text-neutral-400">({count})</span>
                    </button>
                  );
                })}
              </div>
          ...
          {/* Filtres marque / type / origine */}
          <div className="flex flex-wrap items-center gap-2 mb-6">
            <select
              value={filterBrand}
              onChange={(e) => setFilterBrand(e.target.value)}
              className="text-[10px] sm:text-xs uppercase tracking-widest border border-neutral-300 px-2 py-1.5 bg-white"
            >
            ...
            >
              ...
              <option value="Europe">Fabriqué en Europe</option>
            </select>
            ...
            {activeFacetCount > 0 && (
              <button
                onClick={resetFacets}
                className="text-[10px] sm:text-xs uppercase tracking-widest text-pink-600 font-bold underline"
              >
                Effacer ({activeFacetCount})
              </button>
            )}
          </div>
          ...
          {!isSearching && activeFacetCount > 0 && filteredProducts.length === 0 && (
            <p className="text-sm text-neutral-500 mb-6">Aucune pièce ne correspond à ces filtres.</p>
          )}

          {loading ? (
            <p className="text-sm text-neutral-400">Chargement...</p>
          ) : (isSearching ? searchResults : filteredProducts).length === 0 && !isSearching && activeFacetCount === 0 ? (
                  ...
                  <div className="bg-white border border-neutral-200 flex flex-col cursor-pointer hover:border-neutral-900 transition-colors">
                    <div className="relative aspect-[4/5] overflow-hidden bg-stone-100">
                      {p.image_url ? (
                        ...
                        <div className="w-full h-full flex items-center justify-center text-[10px] text-neutral-400 uppercase tracking-widest text-center px-3">
                          Pas encore de photo
                        </div>
                      )}
                      {p.tag && (
          ...
          <div className="fixed inset-0 z-40 flex justify-end">
            ...
            <div className="relative w-full max-w-sm bg-white h-full border-l border-neutral-300 flex flex-col">
              ...
              <div className="border-t border-neutral-300 p-4">
                <p className="text-[10px] text-pink-700 font-semibold uppercase tracking-widest mb-2">
                  Livraison offerte en France métropolitaine dès 30€ d'achat
                </p>
        ...
        {/* Toast après ajout au panier */}
        {justAddedToast && (
          <div className="fixed bottom-4 inset-x-4 sm:inset-x-auto sm:right-6 sm:left-auto sm:w-80 z-50 bg-neutral-900 text-stone-100 p-4 shadow-lg">
            <p className="text-xs uppercase tracking-widest font-bold mb-3">Ajouté au panier ✓</p>
            <div className="flex gap-2">
              <button
                onClick={() => setJustAddedToast(false)}
                className="flex-1 border border-neutral-600 text-stone-100 text-[10px] uppercase tracking-widest font-bold py-2"
              >
                Continuer mes achats
              </button>
              <button
                onClick={() => {
                  setJustAddedToast(false);
                  setCartOpen(true);
                }}
                className="flex-1 bg-pink-600 text-white text-[10px] uppercase tracking-widest font-bold py-2"
              >
                Voir le panier
              </button>
            </div>
