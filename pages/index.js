...
export default function Home() {
  ...
  const filteredProducts = applySort(applyFacets(products.filter((p) => p.category === activeCategory)));

  const searchResults = searchQuery.trim()
    ? applyFacets(products.filter((p) => normalizeText(p.brand + " " + p.name).includes(normalizeText(searchQuery))))
    : [];
