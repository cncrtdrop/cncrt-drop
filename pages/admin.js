  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      const res = await fetch("/api/admin-products", {
        ...
        body: JSON.stringify({ ...form, price: parseFloat(form.price), imageBase64, imageType }),
      });
    ...
    <div className="min-h-screen bg-stone-100 font-mono px-4 sm:px-6 py-8">
      ...
      <div className="max-w-2xl mx-auto">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-3 bg-white border border-neutral-200 p-5 mb-10">
          ...
          <label className="text-xs uppercase tracking-widest text-neutral-500 font-bold mt-1">Photo</label>
          <input type="file" accept="image/*" onChange={handleImageChange} className="text-sm" />
          {imagePreview && (
            <img src={imagePreview} alt="preview" className="w-32 h-40 object-cover border border-neutral-300" />
          )}
