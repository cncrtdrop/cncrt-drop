...
export default function Admin() {
  ...
  async function uploadRemainingPhotos(batchSize = 2) {
    ...
    while (remaining.length > 0) {
      ...
      // Ce paquet a réussi : on le range dans "déjà en ligne" et on retire
      // ces fichiers de la liste "à envoyer".
      setExistingImages((prev) => [...prev, ...data.urls]);
  ...
  async function submitProduct(publishedValue) {
    ...
    try {
      if (newFiles.length > 0) {
        const allUploaded = await uploadRemainingPhotos();
        if (!allUploaded) {
          setSubmitting(false);
          setUploadProgress("");
