...
export default function Admin() {
  ...
  async function moveProduct(product, direction) {
    const sameCategory = products
      .filter((p) => p.category === product.category && p.published)
  ...
  async function moveProductToEdge(product, edge) {
    const sameCategory = products
      .filter((p) => p.category === product.category && p.published)
      .sort((a, b) => (a.sort_order ?? 0) - (b.sort_order ?? 0) || new Date(b.created_at) - new Date(a.created_at));
    ...
    const reordered = [...sameCategory];
    const [moved] = reordered.splice(index, 1);
    if (edge === "top") reordered.unshift(moved);
  ...
  const [adminTab, setAdminTab] = useState("bangers"); // bangers | classiques | kdo | brouillons
  ...
  // Upload les photos restantes par petits paquets. Chaque paquet qui réussit
  // est transféré immédiatement dans "existingImages" — donc si un paquet
  // plus loin échoue, rien de ce qui a déjà été envoyé n'est perdu.
  async function uploadRemainingPhotos(batchSize = 2) {
    let remaining = [...newFiles];
    let uploaded = [...existingImages]; // accumulateur local — la valeur qu'on va vraiment utiliser

    while (remaining.length > 0) {
      const batch = remaining.slice(0, batchSize);
      setUploadProgress(`Envoi de ${batch.length} photo(s)... (${remaining.length} restante(s) au total)`);
      ...
      try {
        ...
        setMessage("Une des photos n'a pas pu être préparée (format non supporté ?). Retire-la et réessaie.");
        return { ok: false, urls: uploaded };
      }
      ...
      try {
        ...
        setMessage("Erreur réseau pendant l'envoi des photos. Ce qui est déjà envoyé est conservé — réessaie.");
        return { ok: false, urls: uploaded };
      }

      if (!res.ok) {
        setMessage("Erreur d'envoi : " + (data.error || "inconnue") + " — ce qui est déjà envoyé est conservé, réessaie.");
        if (res.status === 401) setAuthed(false);
        return { ok: false, urls: uploaded };
      }

      // Ce paquet a réussi : on le range dans "déjà en ligne" (état + accumulateur local)
      // et on retire ces fichiers de la liste "à envoyer".
      uploaded = [...uploaded, ...data.urls];
      setExistingImages(uploaded);
      remaining = remaining.slice(batch.length);
      setNewFiles(remaining);
      setNewPreviews((prev) => prev.slice(batch.length));
    }
    return { ok: true, urls: uploaded };
  }

  async function submitProduct(publishedValue) {
    if (!form.brand.trim() || !form.name.trim() || !form.size.trim() || !form.condition.trim()) {
      setMessage("Marque, nom, taille et état sont obligatoires.");
      return;
    }
    const parsedPrice = parseFloat(form.price);
    if (form.price === "" || isNaN(parsedPrice)) {
      setMessage("Merci d'indiquer un prix valide.");
      return;
    }
    ...
    try {
      ...
      if (newFiles.length > 0) {
        const result = await uploadRemainingPhotos();
        if (!result.ok) {
          setSubmitting(false);
          setUploadProgress("");
          return; // on s'arrête là — rien n'est perdu, il suffit de re-cliquer
        }
    ...
    <div className="min-h-screen bg-stone-100 font-mono px-4 sm:px-6 py-8">
      ...
      <div className="max-w-2xl mx-auto">
        ...
        <form onSubmit={(e) => e.preventDefault()} className="flex flex-col gap-3 bg-white border border-neutral-200 p-5 mb-10">
          ...
          <input
            placeholder="Pays d'origine (lu sur l'étiquette — ex: Portugal, Bangladesh...)"
              ...
              <div className="flex gap-2 flex-wrap">
                {existingImages.map((src, i) => (
                  <div key={i} className="relative">
                    ...
                    <div className="absolute -bottom-2 left-1/2 -translate-x-1/2 flex bg-neutral-900 rounded-full overflow-hidden">
                      <button
                        type="button"
                        disabled={i === 0}
                        onClick={() => moveExistingImage(i, -1)}
                        className="text-white disabled:text-neutral-600 text-xs w-5 h-5 flex items-center justify-center"
                      >
                        ‹
                      </button>
                      <button
                ...
                {newPreviews.map((src, i) => (
                  <div key={i} className="relative">
                    ...
                    <div className="absolute -bottom-2 left-1/2 -translate-x-1/2 flex bg-neutral-900 rounded-full overflow-hidden">
                      ...
                      <button
                        type="button"
                        disabled={i === newPreviews.length - 1}
                        onClick={() => moveNewFile(i, 1)}
                        className="text-white disabled:text-neutral-600 text-xs w-5 h-5 flex items-center justify-center"
                      >
                        ›
        ...
        <p className="text-[10px] text-neutral-400 uppercase tracking-widest mb-4">
          Maintiens la poignée ⠿ et glisse, ou utilise les flèches, pour changer l'ordre
        </p>

        {/* Onglets de catégorie côté admin */}
        <div className="flex gap-4 border-b border-dashed border-neutral-300 mb-4 overflow-x-auto">
          {CATEGORIES.map((cat) => {
            const count = products.filter((p) => p.category === cat.id && p.published).length;
            return (
              <button
                key={cat.id}
                onClick={() => setAdminTab(cat.id)}
                className={
                  "pb-2 text-xs font-bold uppercase tracking-widest border-b-2 -mb-px whitespace-nowrap " +
                  (adminTab === cat.id
          ...
          <button
            onClick={() => setAdminTab("brouillons")}
            className={
              "pb-2 text-xs font-bold uppercase tracking-widest border-b-2 -mb-px whitespace-nowrap " +
              (adminTab === "brouillons"
                ? "border-pink-600 text-neutral-900"
          ...
          >
            Brouillons ({products.filter((p) => !p.published).length})
          </button>
        </div>

        {(() => {
          const isDraftsTab = adminTab === "brouillons";

          const baseItems = isDraftsTab
            ? products
                .filter((p) => !p.published)
                .sort((a, b) => a.category.localeCompare(b.category) || new Date(b.created_at) - new Date(a.created_at))
            : products
                .filter((p) => p.category === adminTab && p.published)
                .sort((a, b) => (a.sort_order ?? 0) - (b.sort_order ?? 0) || new Date(b.created_at) - new Date(a.created_at));

          const items = !isDraftsTab && dragCategory === adminTab && localOrder ? localOrder : baseItems;

          if (items.length === 0) {
            return (
              <p className="text-sm text-neutral-400">
                {isDraftsTab ? "Aucun brouillon pour l'instant." : "Aucune pièce publiée dans cette catégorie."}
              </p>
            );
          }
                ...
                >
                    ...
                    <>
                      ...
                      <div className="flex flex-col gap-0.5">
                        <div className="flex gap-0.5">
                          <button
                            onClick={() => moveProductToEdge(p, "top")}
                            disabled={i === 0}
                            className="text-neutral-400 disabled:text-neutral-200 hover:text-pink-600"
                            aria-label="Tout en haut"
                          >
                            <ChevronsUp size={14} />
                          </button>
                          <button
                        ...
                        <div className="flex gap-0.5">
                          <button
                            onClick={() => moveProductToEdge(p, "bottom")}
                            disabled={i === items.length - 1}
                            className="text-neutral-400 disabled:text-neutral-200 hover:text-pink-600"
                            aria-label="Tout en bas"
                          >
                  ...
                  <div className="flex-1 min-w-[120px] text-xs">
                    <div className="flex items-center gap-1.5">
                      ...
                      {isDraftsTab && (
                        <span className="text-[9px] font-bold uppercase tracking-widest text-neutral-400 border border-neutral-300 px-1 shrink-0">
