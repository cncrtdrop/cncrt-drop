...
export default function Admin() {
  ...
  async function moveProduct(product, direction) {
    ...
    // Déplace l'article dans le tableau, puis réattribue un ordre séquentiel
    // à toute la catégorie — fiable même si les sort_order de départ sont identiques.
    ...
    <div className="min-h-screen bg-stone-100 font-mono px-4 sm:px-6 py-8">
      ...
      <div className="max-w-2xl mx-auto">
        ...
        <p className="text-[10px] text-neutral-400 uppercase tracking-widest mb-4">
          Utilise les flèches pour changer l'ordre d'affichage dans chaque catégorie
        </p>
          ...
          return (
            <div key={cat.id} className="mb-6">
              ...
              <div className="flex flex-col gap-2">
                {items.map((p, i) => (
                  <div key={p.id} className={"flex items-center gap-3 bg-white border p-2 " + (p.published ? "border-neutral-200" : "border-dashed border-neutral-300")}>
                    <div className="flex flex-col">
                      <button
                        disabled={i === 0}
                        onClick={() => moveProduct(p, "up")}
                        className="text-neutral-500 disabled:text-neutral-200 hover:text-pink-600"
                      >
                        <ChevronUp size={16} />
                      </button>
                      <button
                    ...
                    <div className="flex-1 text-xs">
                      <div className="flex items-center gap-1.5">
                        ...
                        {!p.published && (
                          <span className="text-[9px] font-bold uppercase tracking-widest text-pink-600 border border-pink-600/40 px-1">
                            Brouillon
                          </span>
                        )}
                      </div>
